...
@section('main_content')
    <div class="align-items-center border-bottom2 d-flex flex-wrap gap-10 justify-content-between mb-4 pb-3">
        <h4><i class="icofont-plugin"></i> {{ translate('Edit Unit') }}</h4>
    </div>
    <div class="row">
        <div class="col-lg-7 mx-auto">
                        ...
                        @foreach ($languages as $key => $language)
                            <li class="nav-item">
                                <a class="nav-link @if ($language->code == $lang) active border-0 @else bg-light @endif py-3"
                                    href="{{ route('plugin.ecommerce.product.units.edit', ['id' => $unit_details->id, 'lang' => $language->code]) }}">
            ...
            <div class="form-element py-30 mb-30">
                <form action="{{ route('plugin.ecommerce.product.units.update') }}" method="POST">
                    @csrf
                    <div class="form-row mb-20">
                        ...
                        <div class="col-sm-8">
                            <input type="hidden" name="id" value="{{ $unit_details->id }}">
                            <input type="hidden" name="lang" value="{{ $lang }}">
                            <input type="text" name="name" class="theme-input-style"
                                value="{{ $unit_details->translation('name', $lang) }}"
