...
@section('main_content')
    <div class="row">
        ...
        <div
            class="{{ isActivePlugin('wallet') &&auth()->user()->can('Manage Wallet Transactions')? 'col-lg-6': 'col-lg-9' }}   col-12">
            <div class="row">
                            ...
                            <form method="POST" action="{{ route('plugin.wallet.customer.add.deduct') }}">
                                ...
                                <div class="btn-wraper d-flex form-row justify-content-between">
                                    <button type="submit" name="action"
                                        value="{{ config('ecommerce.wallet_entry_type.credit') }}"
                                        class="btn btn-success rounded sm mb-1">
                                        {{ translate('Add Money') }}
                                    </button>
                                    @if ($total_credit > $total_debit)
                                        <button type="submit" name="action"
                                            value="{{ config('ecommerce.wallet_entry_type.debit') }}"
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 'orders' ? 'active' : '' }}"
                        href="{{ route('plugin.ecommerce.customers.details', ['id' => $customer_details->id, 'tab' => 'orders']) }}">
                        {{ translate('Orders') }}
                    </a>
                </li>
                    ...
                    @can('Manage Refund Requests')
                        <li class="nav-item">
                            <a class="nav-link {{ $active_tab == 'refunds' ? 'active' : '' }}"
                                href="{{ route('plugin.ecommerce.customers.details', ['id' => $customer_details->id, 'tab' => 'refunds']) }}">{{ translate('Return Requests') }}</a>
                        </li>
                    @endcan
                @endif
                @can('Manage Product Reviews')
                    <li class="nav-item">
                        <a class="nav-link {{ $active_tab == 'reviews' ? 'active' : '' }}"
                            href="{{ route('plugin.ecommerce.customers.details', ['id' => $customer_details->id, 'tab' => 'reviews']) }}">{{ translate('Product Reviews') }}</a>
                    </li>
                @endcan
                <li class="nav-item">
                    <a class="nav-link {{ $active_tab == 'address' ? 'active' : '' }}"
                        href="{{ route('plugin.ecommerce.customers.details', ['id' => $customer_details->id, 'tab' => 'address']) }}">{{ translate('Addresses') }}</a>
                </li>
                @can('Manage Wishlist Reports')
                    <li class="nav-item">
                        <a class="nav-link {{ $active_tab == 'wishlist' ? 'active' : '' }}"
                            href="{{ route('plugin.ecommerce.customers.details', ['id' => $customer_details->id, 'tab' => 'wishlist']) }}">{{ translate('Wishlists') }}</a>
                    </li>
                @endcan
                @if (isActivePlugin('wallet'))
                    @can('Manage Wallet Transactions')
                        <li class="nav-item">
                            <a class="nav-link {{ $active_tab == 'wallet' ? 'active' : '' }}"
                                href="{{ route('plugin.ecommerce.customers.details', ['id' => $customer_details->id, 'tab' => 'wallet']) }}">{{ translate('Wallet Transactions') }}</a>
                        </li>
                    @endcan
                @endif
            </ul>
                            ...
                            <tbody>
                                @foreach ($customer_orders as $key => $order)
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href="{{ route('plugin.ecommerce.orders.details', ['id' => $order->id]) }}">
                                                {{ $order->order_code }}
                                    ...
                                    <tbody>
                                        @foreach ($return_requests as $key => $order)
                                            <tr>
                                                ...
                                                <td>
                                                    <a
                                                        href="{{ route('plugin.ecommerce.orders.details', ['id' => $order->order_id]) }}">
                                                        {{ $order->order_code }}
                                                    </a>
                                                </td>
                                                ...
                                                <td>
                                                    @if ($order->return_status == config('ecommerce.return_request_status.processing'))
                                                        <span class="badge badge-info">Processing</span>
                                                    @elseif ($order->return_status == config('ecommerce.return_request_status.cancelled'))
                                                        <span class="badge badge-danger">Cancelled</span>
                                                    @elseif ($order->return_status == config('ecommerce.return_request_status.approved'))
                                                ...
                                                <td>
                                                    @if ($order->payment_status == config('ecommerce.return_request_payment_status.refunded'))
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div class="d-flex purchase-price">
                                                    ...
                                                    <div class="ml-1">
                                                        @if ($product->has_variant == config('ecommerce.product_variant.single'))
                                                            <div class="d-flex">
                                                                <div>
                                                        ...
                                                        @php
                                                            ...
                                                            foreach ($variant_array as $com_key => $variant) {
                                                                ...
                                                                if ($variant_com_array[0] === 'color') {
                                                                    ...
                                                                    $option_name = \Plugin\Ecommerce\Models\ProductAttribute::find($variant_com_array[0])->translation('name');
                                            ...
                                            <td>
                                                <div class="dropdown-button">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a
                                                            href="{{ route('plugin.ecommerce.product.edit', ['id' => $product->id, 'lang' => getDefaultLang()]) }}">
