...
use Plugin\Ecommerce\Models\ProductHasTaxs;
...
if (!function_exists('currencyExchange')) {
    ...
    function currencyExchange($value, $formatting = true, $target_currency_id = NULL, $is_html = true)
    {
        ...
        $default_currency_details = Cache::rememberForever('default-currency-details', function () {
            $default_currency_id = Plugin\Ecommerce\Repositories\SettingsRepository::getEcommerceSetting('default_currency');
            return Plugin\Ecommerce\Models\Currency::find($default_currency_id);
        });

        //Convert to target currency
        if ($target_currency_id != null) {
            $target_currency = Plugin\Ecommerce\Models\Currency::where('id', $target_currency_id)->select('conversion_rate')->first();
...
if (!function_exists('currencySymbol')) {
    ...
    {
        $default_currency_id = Plugin\Ecommerce\Repositories\SettingsRepository::getEcommerceSetting('default_currency');
        $default_currency_details = Plugin\Ecommerce\Models\Currency::find($default_currency_id);
        if ($default_currency_details != null) {
