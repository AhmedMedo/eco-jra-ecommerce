...
class RazorpayController extends Controller
{
    ...
    {
        $this->currency = \Plugin\Ecommerce\Repositories\PaymentMethodRepository::configKeyValue(config('ecommerce.payment_methods.razorpay'), 'razorpay_currency');
        $this->total_payable_amount = (new PaymentController())->convertCurrency($this->currency, session()->get('payable_amount'));
        $this->key_id = \Plugin\Ecommerce\Repositories\PaymentMethodRepository::configKeyValue(config('ecommerce.payment_methods.razorpay'), 'razorpay_key_id');
        $this->key_secret = \Plugin\Ecommerce\Repositories\PaymentMethodRepository::configKeyValue(config('ecommerce.payment_methods.razorpay'), 'razorpay_key_secret');
    }
    ...
    public function index()
    {
        try {
            ...
            return view('plugin/ecommerce::payments.gateways.razorpay.index', [
                'key_id' => $this->key_id,
                'order' => [
                    'id' => $orderId,
                    'amount' => $orderAmount,
