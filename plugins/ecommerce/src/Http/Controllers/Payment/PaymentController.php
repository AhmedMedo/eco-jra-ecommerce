    ...
    public function createPayment(Request $request, $payment_method)
    {
        ...
        if (session()->has('payment_type') && session()->has('payable_amount')) {
            ...
            if ($payment_method == 'stripe') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\StripeController)->index();
            }

            if ($payment_method == 'paypal') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\PaypalController)->index();
            }

            if ($payment_method == 'paddle') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\PaddleController)->index();
            }

            if ($payment_method == 'sslcommerz') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\SSLCommerzController)->index();
            }

            if ($payment_method == 'paystack') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\PaystackController)->index();
            }

            if ($payment_method == 'razorpay') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\RazorpayController)->index();
            }

            if ($payment_method == 'mollie') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\MollieController)->index();
            }

            if ($payment_method == 'gpay') {
                return (new \Plugin\Ecommerce\Http\Controllers\Payment\GpayController)->index();
            }
            return redirect('/404');
        } else {
            return redirect('/404');
