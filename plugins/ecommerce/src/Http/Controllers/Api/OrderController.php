...
use Plugin\Ecommerce\Http\Resources\CityCollection;
use Plugin\Ecommerce\Http\Resources\OrderCollection;
use Plugin\Ecommerce\Http\Resources\StateCollection;
use Plugin\Ecommerce\Http\Resources\CountryCollection;
use Plugin\Ecommerce\Http\Requests\GuestCheckoutRequest;
use Plugin\Ecommerce\Http\Requests\ProductReturnRequest;
use Plugin\Ecommerce\Http\Resources\SingleOrderCollection;
use Plugin\Ecommerce\Repositories\PaymentMethodRepository;
use Plugin\Ecommerce\Http\Requests\AttachmentUploadRequest;
use Plugin\Ecommerce\Http\Resources\PaymentMethodCollection;
use Plugin\Ecommerce\Http\Resources\RefundRequestCollection;
...
class OrderController extends Controller
{
    ...
    public function activePaymentMethods(Request $request)
    {
        try {
            ...
            if (!$is_available_cod) {
                $query = $query->whereNot('id', config('ecommerce.payment_methods.cod'));
            }
    ...
    public function isAvailableCodInCheckout($product_id, $city_id, $pickup_point)
    {
        ...
        //Cod in any location 
        if ($product_info->is_active_cod == config('settings.general_status.active') && $product_info->cod_location_type == config('ecommerce.cod_location.anywhere')) {
            return true;
        }
        //Cod in custom location
        if ($product_info->is_active_cod == config('settings.general_status.active') && $product_info->cod_location_type == config('ecommerce.cod_location.custom')) {
    ...
    public function checkAvailabilityCartItem($item)
    {
        ...
        if ($product_details->has_variant == config('ecommerce.product_variant.single')) {
        ...
        if ($product_details->has_variant == config('ecommerce.product_variant.variable')) {
        ...
        if (isActivePlugin('multivendor') && $product_details->is_approved != config('settings.general_status.active')) {
            return config('settings.general_status.in_active');
        }

        //check supplier and shop status
        if (isActivePlugin('multivendor') && $item['seller'] != null) {
    ...
    public function applyCoupon(Request $request)
    {
        ...
        try {
            ...
            $discounted_amount = 0;
            //Flat Discount
            if ($coupon_details->discount_type == config('ecommerce.amount_type.flat')) {
