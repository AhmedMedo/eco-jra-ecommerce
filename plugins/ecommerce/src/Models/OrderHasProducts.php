...
use Plugin\Ecommerce\Observer\SellerEarningObserver;
...
class OrderHasProducts extends Model
{
    ...
    {
        if ($this->payment_status == config('ecommerce.order_payment_status.paid')) {
            return 'paid';
        } else {
            return 'unpaid';
        }
    ...
    public function canReturn()
    {
        ...
        if ($this->return_status != config('ecommerce.product_return_status.available')) {
            return config('settings.general_status.in_active');
        }
    ...
    {
        if ($this->return_status == config('ecommerce.product_return_status.not_available')) {
            return
                [
        ...
        } else if ($this->return_status == config('ecommerce.product_return_status.available') && $this->canReturn() == config('settings.general_status.active')) {
            return
                [
                    'status' => $this->return_status,
                    'class' => 'primary',
        ...
        } else if ($this->return_status == config('ecommerce.product_return_status.returned')) {
            return
                [
                    'status' => $this->return_status,
                    'class' => 'success',
        ...
        } else if ($this->return_status == config('ecommerce.product_return_status.processing')) {
            return
                [
                    'status' => $this->return_status,
                    'class' => 'info',
        ...
        } else if ($this->return_status == config('ecommerce.product_return_status.return_cancel')) {
            return
                [
                    'status' => $this->return_status,
                    'class' => 'danger',
    ...
    public function estimateDeliveryTime()
    {
        if ($this->shipping_rate_info != null && $this->shipping_rate_info->shipping_time != null) {
            $min_time = "";
            if ($this->shipping_rate_info->shipping_time->min_unit == config('ecommerce.time_unit.Days')) {
                $min_time = $this->created_at->addDays($this->shipping_rate_info->shipping_time->min_value);
            }
            if ($this->shipping_rate_info->shipping_time->min_unit == config('ecommerce.time_unit.Hours')) {
                $min_time = $this->created_at->addHours($this->shipping_rate_info->shipping_time->min_value);
            }
            if ($this->shipping_rate_info->shipping_time->min_unit == config('ecommerce.time_unit.Minutes')) {
                $min_time = $this->created_at->addMinutes($this->shipping_rate_info->shipping_time->min_value);
            }
            $max_time = "";
            if ($this->shipping_rate_info->shipping_time->max_unit == config('ecommerce.time_unit.Days')) {
                $max_time = $this->created_at->addDays($this->shipping_rate_info->shipping_time->max_value);
            }
            if ($this->shipping_rate_info->shipping_time->max_unit == config('ecommerce.time_unit.Hours')) {
                $max_time = $this->created_at->addHours($this->shipping_rate_info->shipping_time->max_value);
            }
            if ($this->shipping_rate_info->shipping_time->max_unit == config('ecommerce.time_unit.Minutes')) {
    ...
    {
        if ($this->delivery_status == config('ecommerce.order_delivery_status.cancelled') || $this->delivery_status != config('ecommerce.order_delivery_status.pending')) {
            return config('settings.general_status.in_active');
        }
