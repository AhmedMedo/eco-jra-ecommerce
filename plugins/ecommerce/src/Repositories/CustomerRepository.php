...
use Plugin\Ecommerce\Mail\CustomerResetEmail;
use Plugin\Ecommerce\Models\CustomerWishlist;
use Plugin\Ecommerce\Mail\CustomerForgotPassword;
use Plugin\Ecommerce\Mail\CustomerEmailVerification;
...
class CustomerRepository
{
    ...
    public function customerDashboardDetails($customer_id)
    {
        $data = [
            ...
            'total_successfull_order' => Orders::where('customer_id', $customer_id)->where('delivery_status', config('ecommerce.order_delivery_status.delivered'))->count(),
            'total_pending_order' => Orders::where('customer_id', $customer_id)->whereNotIn('delivery_status', [config('ecommerce.order_delivery_status.delivered')])->count(),
