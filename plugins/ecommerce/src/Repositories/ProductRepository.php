...
class ProductRepository
{
    ...
    public function productManagement($request, $seller_id = null, $product_owner = null)
    {
        ...
        if ($product_owner != null && $product_owner == 'seller') {
            $query = $query->whereHas('seller', function ($q) {
                $q->where('user_type', config('ecommerce.user_type.seller'));
            });
        }
        //In house products
        if ($product_owner != null && $product_owner == 'inhouse') {
            $query = $query->whereHas('seller', function ($q) {
                $q->whereNull('user_type')
                    ->orWhere('user_type', config('ecommerce.user_type.admin'));
            });
        }
    ...
    public function storeNewProduct($request)
    {
        try {
            ...
            if (isActivePlugin('multivendor') && auth()->user()->user_type == config('ecommerce.user_type.seller') && getGeneralSetting('product_auto_approve') != config('settings.general_status.active')) {
            ...
            $product->product_type  = config('ecommerce.product_type.physical_product');
            ...
            //store product price
            if ($request['product_type'] == config('ecommerce.product_variant.single')) {
            ...
            //Store shipping info
            if (getEcommerceSetting('shipping_option') == config('ecommerce.shipping_cost_options.profile_wise_rate')) {
                $this->storeProductShippingInfo($product->id, $request);
            }


            //Store cod areas
            if ($request->has('cash_on_delivery') && $request['cod_location'] == config('ecommerce.cod_location.custom')) {
            ...
            if (auth()->user()->user_type == config('ecommerce.user_type.seller')) {
    ...
    public function updateProductPrice($request)
    {
        try {
            DB::beginTransaction();
            if ($request['has_variant'] == config('ecommerce.product_variant.single')) {
    ...
    public function filterProducts($request, $seller_id = null, $product_ids = [])
    {
        ...
        if (!isActivePlugin('multivendor') && $seller_id == null) {
            $query = $query->whereHas('seller', function ($q) {
                $q->where('user_type', config('ecommerce.user_type.admin'));
            });
        }
        ...
        $priceCondition = function ($q, $request, $product_ids) {
            $q->whereBetween('unit_price', [$request['min_price'], $request['max_price']]);
        ...
        if ($request->has('min_price') && $request->has('max_price')) {
            ...
            })->orWhereHas('variations', function ($q) use ($request, $priceCondition, $product_ids) {
                $priceCondition($q, $request, $product_ids);
            });
        }
     ...
     * @return Collections
     */
    public function relatedProducts($id, $number = 8)
    {
