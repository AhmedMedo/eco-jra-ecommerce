...
class PageBuilderService
{
    ...
    public function __construct()
    {
        ...
        if (!File::exists(base_path("themes/{$this->active_theme->location}builder-assets/css"))) {
            File::makeDirectory(base_path("themes/{$this->active_theme->location}builder-assets/css"), 0755, true);
        }
    }
    ...
    public function makeBackgroundAdvanceCss($data, $type, $file_name)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($key == 'padding_right') {
                $css['html.rtl ' . $id]['padding-left'] = $value . ';';
            }

            if ($key == 'padding_left') {
                $css['html.rtl ' . $id]['padding-right'] = $value . ';';
            }

            if ($key == 'margin_right') {
                $css['html.rtl ' . $id]['margin-left'] = $value . ';';
            }

            if ($key == 'margin_left') {
                $css['html.rtl ' . $id]['margin-right'] = $value . ';';
            }
        }
    ...
    {
        $path = base_path("themes/{$this->active_theme->location}builder-assets/css/{$file_name}.json");
        $page_css = null;
    ...
    {
        $path = base_path("themes/{$this->active_theme->location}builder-assets/css/{$data['page_permalink']}.json");
    ...
    {
        $css_path = base_path("themes/{$this->active_theme->location}builder-assets/css/{$file_name}.css");
