...
class PageBuilderController extends Controller
{
    ...
    public function imageUpload(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $file->move("themes/{$active_theme->location}builder-assets/text-editor/", $image);
            $path = asset("themes/{$active_theme->location}builder-assets/text-editor/" . $image);
    ...
    public function showModal(Request $request)
    {
        try {
            ...
            $modal = view('plugin/pagebuilder::builders.includes.banner-modal', ['details' => $details, 'key' => $request->key])->render();
    ...
    public function showModalFeature(Request $request)
    {
        try {
            $details = empty($request->details) ? null : $request->details;
            $modal = view('plugin/pagebuilder::builders.includes.feature-modal', ['details' => $details, 'key' => $request->key, 'lang' => $request->lang])->render();
            return BuilderHelper::jsonResponse(200, '', $modal);
        } catch (\Exception $e) {
