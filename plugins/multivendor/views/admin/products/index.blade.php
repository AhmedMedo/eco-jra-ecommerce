            ...
            <div class="card mb-30">
                <div class="card-body border-bottom2 mb-20">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="font-20">{{ translate('Seller Products') }}</h4>
                    </div>
                </div>
                <div class="px-2 filter-area d-flex align-items-center">
                    <!--Filter area-->
                    <form method="get" action="{{ route('plugin.multivendor.admin.seller.products.list') }}">
                        ...
                        <select class="theme-input-style mb-2" name="has_variation">
                            ...
                            <option value="{{ config('ecommerce.product_variant.variable') }}"
                                @selected(request()->has('has_variation') && request()->get('has_variation') == config('ecommerce.product_variant.variable'))>
                                {{ translate('Variant Product') }}
                            </option>
                            <option value="{{ config('ecommerce.product_variant.single') }}" @selected(request()->has('has_variation') && request()->get('has_variation') == config('ecommerce.product_variant.single'))>
                    ...
                    @if (request()->has('search_key') || request()->has('payment_status'))
                        <a class="btn long btn-danger" href="{{ route('plugin.multivendor.admin.seller.products.list') }}">
                            ...
                            @if ($products->count() > 0)
                                @foreach ($products as $key => $product)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="product-title text-capitalize">
                                                ...
                                                @if ($product->seller != null)
                                                    <p>{{ translate('Seller') }}
                                                        <a href="{{ route('plugin.multivendor.admin.seller.details', ['id' => $product->seller->id]) }}"
                                                        ...
                                                        <div class="d-flex">
                                                            <div>
                                                                @if ($product->discount_amount != null)
                                                                    @if ($product->discount_type == config('ecommerce.amount_type.flat'))
                                                        ...
                                                        @php
                                                            ...
                                                            foreach ($variant_array as $com_key => $variant) {
                                                                ...
                                                                if ($variant_com_array[0] === 'color') {
                                                                    ...
                                                                    $option_property = \Plugin\Ecommerce\Models\ProductAttribute::select(['id', 'name'])->find($variant_com_array[0]);
                                                                    $option_name = $option_property != null ? $option_property->translation('name') : '';
                                                                    $choice_property = \Plugin\Ecommerce\Models\AttributeValues::select(['id', 'name'])->find($variant_com_array[1]);
                                        ...
                                        <td>
                                            <label class="switch glow primary medium">
                                                <input type="checkbox" class="change-approval-status"
                                                    data-product="{{ $product->id }}"
                                                    {{ $product->is_approved == config('settings.general_status.active') ? 'checked' : '' }}>
                                                <span class="control"></span>
                                            </label>
                                        </td>
                                        <td>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <form method="POST" action="{{ route('plugin.ecommerce.product.delete') }}">
                        @csrf
    ...
    <script>
        (function($) {
            ...
            $('.quick-action').on('click', function(e) {
                ...
                let id = $(this).data('id');
                $.post('{{ route('plugin.ecommerce.product.quick.action.modal.view') }}', {
                    _token: '{{ csrf_token() }}',
                    action: action,
                    id: id
                }, function(data) {
            ...
            $('.fire-bulk-action').on('click', function(e) {
                ...
                if (action != 'null') {
                    ...
                    if (selected_items.length > 0) {
                        $.post('{{ route('plugin.ecommerce.product.bulk.action') }}', {
                            _token: '{{ csrf_token() }}',
            ...
            $('.change-featured').on('click', function(e) {
                ...
                $.post('{{ route('plugin.ecommerce.product.status.featured.update') }}', {
                    _token: '{{ csrf_token() }}',
                    id: id
                }, function(data) {
                    location.reload();
            ...
            $('.change-status').on('click', function(e) {
                ...
                let $this = $(this);
                let id = $this.data('product');
                $.post('{{ route('plugin.ecommerce.product.status.update') }}', {
                    _token: '{{ csrf_token() }}',
                    id: id
                }, function(data) {
                    location.reload();
            ...
            /**
             * 
             * Change  product approval status 
             * 
             * */
            $('.change-approval-status').on('click', function(e) {
                e.preventDefault();
                let $this = $(this);
                let id = $this.data('product');
