...
@endsection
@section('custom_css')
    <link rel="stylesheet" href="{{ asset('web-assets/backend/plugins/daterangepicker/daterangepicker.css') }}">
@endsection
            ...
            <div class="card mb-30">
                <div class="card-body border-bottom2 mb-20">
                    <div class="d-sm-flex justify-content-between align-items-center">
                        <h4 class="font-20">{{ translate('Seller Orders') }}</h4>
                    </div>

                </div>
                <!--Filter Counter-->
                <div class="px-2 filter-area d-flex align-items-center mb-20">
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list') }}"
                        class="btn sm btn-info">{{ translate('All') }}
                        ({{ $order_counter != null ? $order_counter['all'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['delivery_status' => config('ecommerce.order_delivery_status.pending')]) }}"
                        class="btn sm btn-primary">{{ translate('Pending') }}
                        ({{ $order_counter != null ? $order_counter['pending'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['delivery_status' => config('ecommerce.order_delivery_status.processing')]) }}"
                        class="btn sm btn-info">{{ translate('Processing') }}
                        ({{ $order_counter != null ? $order_counter['processing'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['delivery_status' => config('ecommerce.order_delivery_status.ready_to_ship')]) }}"
                        class="btn sm btn-success">{{ translate('Ready to ship') }}
                        ({{ $order_counter != null ? $order_counter['ready_to_ship'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['delivery_status' => config('ecommerce.order_delivery_status.shipped')]) }}"
                        class="btn sm">{{ translate('To Shipped') }}
                        ({{ $order_counter != null ? $order_counter['shipped'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['delivery_status' => config('ecommerce.order_delivery_status.delivered')]) }}"
                        class="btn sm btn-success">{{ translate('Delivered') }}
                        ({{ $order_counter != null ? $order_counter['delivered'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['payment_status' => config('ecommerce.order_payment_status.unpaid')]) }}"
                        class="btn sm btn-warning">{{ translate('Unpaid') }}
                        ({{ $order_counter != null ? $order_counter['unpaid'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['payment_status' => config('ecommerce.order_payment_status.paid')]) }}"
                        class="btn sm btn-success">{{ translate('Paid') }}
                        ({{ $order_counter != null ? $order_counter['paid'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.multivendor.admin.seller.order.list', ['delivery_status' => config('ecommerce.order_delivery_status.cancelled')]) }}"
                ...
                <div class="px-2 filter-area d-flex align-items-center">
                    ...
                    <select class="theme-input-style" id="bulkActionSelector">
                        ...
                        <option value="d-{{ config('ecommerce.order_delivery_status.processing') }}">
                            {{ translate('Change status to processing') }}
                        </option>
                        <option value="d-{{ config('ecommerce.order_delivery_status.ready_to_ship') }}">
                            {{ translate('Change status to ready to ship') }}
                        </option>
                        <option value="d-{{ config('ecommerce.order_delivery_status.shipped') }}">
                            {{ translate('Change status to shipped') }}
                        </option>
                        <option value="d-{{ config('ecommerce.order_delivery_status.delivered') }}">
                            {{ translate('Change status to delivered') }}
                        </option>
                        <option value="p-{{ config('ecommerce.order_payment_status.paid') }}">
                            {{ translate('Change status to paid') }}
                        </option>
                        <option value="p-{{ config('ecommerce.order_payment_status.unpaid') }}">
                            {{ translate('Change status to unpaid') }}
                        </option>
                        <option value="d-{{ config('ecommerce.order_delivery_status.cancelled') }}">
                            {{ translate('Move to trash') }}
                        </option>
                    </select>
                    <button class="btn long btn-danger fire-bulk-action"
                        href="{{ route('plugin.multivendor.admin.seller.order.list') }}"
                        type="submit">{{ translate('Apply') }}
                    </button>
                    <!--End bulk actions-->
                    <!--Filter area-->
                    <form method="get" action="{{ route('plugin.multivendor.admin.seller.order.list') }}">
                        ...
                        <select class="theme-input-style mb-2" name="delivery_status">
                            ...
                            <option value="{{ config('ecommerce.order_delivery_status.pending') }}"
                                @selected(request()->has('delivery_status') && request()->get('delivery_status') == config('ecommerce.order_delivery_status.pending'))>
                                {{ translate('Pending') }}</option>
                            <option value="{{ config('ecommerce.order_delivery_status.processing') }}"
                                @selected(request()->has('delivery_status') && request()->get('delivery_status') == config('ecommerce.order_delivery_status.processing'))>
                                {{ translate('Processing') }}</option>
                            <option value="{{ config('ecommerce.order_delivery_status.ready_to_ship') }}"
                                @selected(request()->has('delivery_status') && request()->get('delivery_status') == config('ecommerce.order_delivery_status.ready_to_ship'))>
                                {{ translate('ready_to_ship') }}</option>
                            <option value="{{ config('ecommerce.order_delivery_status.shipped') }}"
                                @selected(request()->has('delivery_status') && request()->get('delivery_status') == config('ecommerce.order_delivery_status.shipped'))>
                                {{ translate('Shipped') }}</option>
                            <option value="{{ config('ecommerce.order_delivery_status.delivered') }}"
                                @selected(request()->has('delivery_status') && request()->get('delivery_status') == config('ecommerce.order_delivery_status.delivered'))>
                                {{ translate('Delivered') }}</option>

                            <option value="{{ config('ecommerce.order_delivery_status.cancelled') }}"
                                @selected(request()->has('delivery_status') && request()->get('delivery_status') == config('ecommerce.order_delivery_status.cancelled'))>
                        ...
                        <select class="theme-input-style mb-2" name="payment_status">
                            ...
                            <option value="{{ config('ecommerce.order_payment_status.paid') }}"
                                @selected(request()->has('payment_status') && request()->get('payment_status') == config('ecommerce.order_payment_status.paid'))>{{ translate('Paid') }}
                            </option>
                            <option value="{{ config('ecommerce.order_payment_status.unpaid') }}"
                                @selected(request()->has('payment_status') && request()->get('payment_status') == config('ecommerce.order_payment_status.unpaid'))>{{ translate('Unpaid') }}
                            </option>
                        </select>
                    ...
                    @if (request()->has('order_code') ||
                        ...
                        <a class="btn long btn-danger" href="{{ route('plugin.multivendor.admin.seller.order.list') }}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->customer_name != null)
                                                <a
                                                    href="{{ route('plugin.ecommerce.customers.details', ['id' => $order->customer_id]) }}">
                                                    {{ $order->customer_name }}
                                                </a>
                                            @else
                                        ...
                                        <td>
                                            @if ($order->delivery_status == config('ecommerce.order_delivery_status.pending'))
                                        ...
                                        <td>
                                            <div class="dropdown-button">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a
                                                        href="{{ route('plugin.ecommerce.orders.details', ['id' => $order->id]) }}">{{ translate('Details') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <form method="POST" action="{{ route('plugin.ecommerce.orders.cancel') }}">
                        @csrf
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <form method="POST" action="{{ route('plugin.ecommerce.orders.accept') }}">
                        @csrf
...
@section('custom_scripts')
    <script src="{{ asset('web-assets/backend/plugins/moment/moment.min.js') }}"></script>
    <script type="text/javascript"
        src="{{ asset('web-assets/backend/plugins/daterangepicker/daterangepicker.js') }}"></script>
    <script>
        (function($) {
            "use strict";
            /**
            ...
            $('.fire-bulk-action').on('click', function(e) {
                ...
                if (action != "") {
                    ...
                    if (selected_items.length > 0) {
                        $.post('{{ route('plugin.ecommerce.orders.bulk.action') }}', {
                            _token: '{{ csrf_token() }}',
                            data: data
                        }, function(data) {
                            location.reload();
            ...
            $('.status-details-btn').on('click', function(e) {
                ...
                e.preventDefault();
                let id = $(this).data('order');
                $.post('{{ route('plugin.ecommerce.orders.status.details') }}', {
                    _token: '{{ csrf_token() }}',
                    id: id
                }, function(data) {
