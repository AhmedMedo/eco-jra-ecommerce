...
class WalletRepository
{
    ...
    {
        return PaymentMethods::whereNotIn('id', [config('ecommerce.payment_methods.cod')])->get()->map(function ($item) {
            return [
                'name' => $item->name,
    ...
    public function storeOfflinePayment($request)
    {
        try {
            ...
            if ($request['method_type'] == config('ecommerce.offline_payment_type.bank')) {
    ...
    public function updateOfflinePayment($request)
    {
        try {
            ...
            if ($request['method_type'] != config('ecommerce.offline_payment_type.bank') && $payment_method->bank_info != null) {
    ...
    public function storeWalletTraction($data)
    {
        try {
            ...
            $message = "";
            if ($data['entry_type'] == config('ecommerce.wallet_entry_type.credit')) {
                $message = currencyExchange($data['recharge_amount']) . " credited to your wallet";
            }
            if ($data['entry_type'] == config('ecommerce.wallet_entry_type.debit')) {
    ...
    public function customerWalletSummary($customer_id)
    {
        try {
            ...
            $total_credit = collect($customer_wallet_transactions)->count() > 0 ? $customer_wallet_transactions->where('entry_type', config('ecommerce.wallet_entry_type.credit'))
                ->where('status', config('ecommerce.wallet_transaction_status.accept'))
                ->sum('recharge_amount') : 0;
            $total_debit = collect($customer_wallet_transactions)->count() > 0 ? $customer_wallet_transactions->where('entry_type', config('ecommerce.wallet_entry_type.debit'))
                ->where('status', config('ecommerce.wallet_transaction_status.accept'))
                ->sum('recharge_amount') : 0;
            $total_pending = collect($customer_wallet_transactions)->count() > 0 ? $customer_wallet_transactions->where('entry_type', config('ecommerce.wallet_entry_type.credit'))
                ->where('status', config('ecommerce.wallet_transaction_status.pending'))
                ->sum('recharge_amount') : 0;
            $data = [];
    ...
    public function bulkStatusUpdate($request)
    {
        try {
            ...
            foreach ($items as $item) {
                ...
                if ($request['data']['action'] == config('ecommerce.wallet_transaction_status.declined')) {
                    $message = "Your wallet transaction has been declined";
                }
                if ($request['data']['action'] == config('ecommerce.wallet_transaction_status.accept')) {
                    $message = "Your wallet transaction has been accepted";
                }
                if ($request['data']['action'] == config('ecommerce.wallet_transaction_status.pending')) {
