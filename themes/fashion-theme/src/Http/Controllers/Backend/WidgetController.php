...
namespace Theme\FashionTheme\Http\Controllers\Backend;
...
use Theme\FashionTheme\Repositories\WidgetRepository;

class WidgetController extends Controller
{
  protected $widget_repository;

  public function __construct(WidgetRepository $widget_repository)
  {
    $this->widget_repository = $widget_repository;
    getThemeWidgetNameAsArray('fashion_theme_widget'); //theme widget
    getThemeSidebarNameAsArray('fashion_theme_sidebar'); //theme sidebars
  }
  ...
  public function widgets()
  {
    try {
      ...
      $widgets = $widgets = DB::table('tl_widgets')
        ->where($condition)
        ->get();

      $sidebars = $this->widget_repository->getSidebar($condition)->get();

      return view('theme/fashion-theme::backend.widgets.widget', compact('widgets', 'sidebars'));
    } catch (\Exception $e) {
  ...
  public function getWidgetInputFields(Request $request)
  {
    try {
      if (isset($request->sidebar_has_widget_id) && is_numeric($request->sidebar_has_widget_id)) {
        ...
        $html = view('theme/fashion-theme::backend.widgets.widget-forms.' . $widget_name, compact('value', 'sidebar_has_widget_id', 'widget_id', 'lang'))->render();
