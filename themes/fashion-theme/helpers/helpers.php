...
use Theme\FashionTheme\Models\TlWidget;
use Illuminate\Support\Facades\Session;
use Theme\FashionTheme\Models\TlThemeSidebar;
use Theme\FashionTheme\Repositories\BlogRepository;
use Theme\FashionTheme\Models\TlSidebarWidgetHasValue;
...
if (!function_exists('getAllFonts')) {
    ...
    {
        $url = config('fashion-theme.google_font_api_key.url');
        ...
        if ($custom_options[$custom_font_number] == '1') {
            ...
            if ($font_woff_file != '') {
                if (isset($customFontFace['src']) && $customFontFace['src'] != '') {
                    $customFontFace['src'] = $customFontFace['src'] . ',' . "url('" . asset('themes/fashion-theme' . $custom_font_number . '/' . $font_woff_file) . "')";
                } else {
                    $customFontFace['src'] = "url('" . asset('themes/fashion-theme' . $custom_font_number . '/' . $font_woff_file) . "')";
                }
            }

            if ($font_ttf_file != '') {
                if (isset($customFontFace['src']) && $customFontFace['src'] != '') {
                    $customFontFace['src'] = $customFontFace['src'] . ',' . "url('" . asset('themes/fashion-theme' . $custom_font_number . '/' . $font_ttf_file) . "')";
                } else {
                    $customFontFace['src'] = "url('" . asset('themes/fashion-theme' . $custom_font_number . '/' . $font_ttf_file) . "')";
                }
            }

            if ($font_eot_file != '') {
                if (isset($customFontFace['src']) && $customFontFace['src'] != '') {
                    $customFontFace['src'] = $customFontFace['src'] . ',' . "url('" . asset('themes/fashion-theme' . $custom_font_number . '/' . $font_eot_file) . "')";
                } else {
                    $customFontFace['src'] = "url('" . asset('themes/fashion-theme' . $custom_font_number . '/' . $font_eot_file) . "')";
                }
            }
        }
...
if (!function_exists('getThemeWidgetNameAsArray')) {
    ...
    {
        $all_settings_name = config('fashion-theme.' . $theme_widget_names);
        $names = [];
        $ids = [];
...
if (!function_exists('getThemeSidebarNameAsArray')) {
    ...
    {
        $all_settings_name = config('fashion-theme.' . $theme_sidebar_names);
        $names = [];
        $ids = [];
        for ($i = 0; $i < sizeof($all_settings_name); $i++) {
