...
class MenuController extends Controller
{
  ...
  {
    $all_menu_positions = DB::table('tl_themes')
    ...
    $menu_items = DB::table('tl_menu_items')
    ...
    $menu_groups = $this->menu_repository->getAllMenuGroups();

    return view(
      'core::base.menu.index',
      [
        'menu_items' => $menu_items,
        'all_menu_positions' => $all_menu_positions,
  ...
  public function addMenuGroup(Request $request)
  {
    ...
    try {
      ...
      $menu_group->name = $request['menu_name'];
      $menu_group->saveOrFail();
      ...
      for ($i = 0; $i < sizeof($menus); $i++) {
        if ($menus[$i]['id'] != "-1") {
          ...
          if (isset($menus[$i]['menu_type_id'])) {
            ...
            $menu->url = xss_clean($menus[$i]['url']);
          } else {
            $menu->url = xss_clean($menus[$i]['url']);
          }
          ...
          $menu->title = xss_clean($menus[$i]['title']);
  ...
  public function searchProductCategoryByKeywords(Request $request)
  {
    try {
      ...
      return view('plugin/ecommerce::menu.partial.filtered_product_categories', compact('all_product_categories'));
    } catch (\Exception $e) {
      return response()->json([
        'success' => false,
        'message' => translate("Invalid Request")
  ...
  public function searchProductTagByKeywords(Request $request)
  {
    try {
      ...
      return view('plugin/ecommerce::menu.partial.filtered_product_tags', compact('all_product_tags'));
    } catch (\Exception $e) {
      return response()->json([
        'success' => false,
        'message' => translate("Invalid Request")
  ...
  public function searchProductBrandByKeywords(Request $request)
  {
    try {
      ...
      return view('plugin/ecommerce::menu.partial.filtered_product_brands', compact('all_product_brands'));
    } catch (\Exception $e) {
      return response()->json([
        'success' => false,
        'message' => translate("Invalid Request")
