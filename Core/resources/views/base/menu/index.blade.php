@php
    ...
    //Pages
    $all_pages = (new \Core\Repositories\PageRepository())->getAllPages();
    $all_recent_pages = (new \Core\Repositories\PageRepository())->getAllRecentPages();
    
    //Blogs Categories
    $all_categories = (new \Core\Repositories\BlogRepository())->getAllCategories();
    $all_recent_categories = (new \Core\Repositories\BlogRepository())->getAllRecentCategories();
    
    //Blogs Posts
    $all_posts = (new \Core\Repositories\BlogRepository())->getAllPosts();
    $all_recent_posts = (new \Core\Repositories\BlogRepository())->getAllRecentPosts();
    
    //Blog tags
    $all_tags = (new \Core\Repositories\BlogRepository())->getAllTagsForMenu();
    $all_recent_tags = (new \Core\Repositories\BlogRepository())->getAllRecentTagsForMenu();
@endphp
                    ...
                    <select class="form-control w-100" id="language" onchange="getMenuStructure()">
                        @foreach ($languages as $lang)
                            <option value="{{ $lang->id }}" class="text-uppercase" id="lang_{{ $lang->id }}"
                                @selected($lang->code == getDefaultLang())>
                                {{ $lang->name }}
                            </option>
                        @endforeach
                    </select>
...
@section('custom_scripts')
    ...
    <script src="{{ asset('web-assets/backend/js/menu_tree_sortable.js') }}"></script>
    <script src="{{ asset('web-assets/backend/js/menu_script.js') }}"></script>
