@php
    $placeholder_image = getPlaceHolderImagePath();
    if ($data == null || $data == '') {
        $data = [];
    } else {
        ...
        <div id="multi_input_container_{{ $indicator }}" class="d-flex flex-wrap gap-10">
            @if (sizeof($data) == 0)
                <div class="preview-image-wrapper" id="div_preview">
                    <img src="{{ $placeholder_image }}" alt="multuple-file-input" class="preview_image"
                        id="preview_image" />
                </div>
            @endif
            @if (sizeof($data) > 0)
                @for ($i = 0; $i < sizeof($data); $i++)
                    @if ($data[$i] != null)
                        <div class="preview-image-wrapper"
                            ...
                            <img src="{{ getFilePath(getFileDetails($data[$i])->id) }}" alt="multuple-file-input"
                                class="preview_image"
                            ...
                            <button type="button" title="Remove image" class="remove-btn style--three"
                                ...
                                onclick="removeSelectionForMultiSelect('#preview_{{ $input }}_{{ $indicator }}_{{ $data[$i] }},#{{ $input }}_{{ $indicator }},#remove_{{ $input }}_{{ $indicator }}_{{ $data[$i] }},#div_preview_{{ $input }}_{{ $indicator }}_{{ $data[$i] }}',{{ $data[$i] }})">
