...
<script src="<?php echo e(asset('web-assets/backend/plugins/dropzone/dropzone.min.js')); ?>"></script>
<!-- ======= BEGIN GLOBAL MANDATORY SCRIPTS ======= -->
...
<script>
    ...
    function initDropzone() {
        "use strict";
        let total_files = 0
        let chunk_size = <?php echo e($chink_size); ?>
    ...
    function filtermediaOnEdit(selected_media_files, ids) {
        ...
        $.post("<?php echo e(route('core.filter.media.list')); ?>", {
                _token: '<?php echo e(csrf_token()); ?>',
                file_type: file_type,
                media_type: media_type,
                search_input: search_input,
                ...
                for (let i = 0; i < all_files.length; i++) {
                    for (let j = 0; j < selected_file_id.length; j++) {
                        if (all_files[i].id == selected_file_id[j]) {
                            let path = '<?php echo e(project_asset('/')); ?>' + "/" + all_files[i].path
    ...
    function removeSelection(ids) {
        ...
        $(rmoveableIds[0]).attr('src', '<?php echo e(getPlaceHolderImagePath()); ?>')
        $(rmoveableIds[1]).val('')
        $(rmoveableIds[2]).hide()
    }
...
<?php /**PATH /home/medo/work/eco-jara/fashly/Core/resources/views/base/layouts/script.blade.php ENDPATH**/ ?>