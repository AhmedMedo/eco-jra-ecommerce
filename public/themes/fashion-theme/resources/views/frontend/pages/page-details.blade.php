...
@section('builder-css-link')
    @if ($page_details)
        @php
            ...
            $builder_css_file = base_path("themes/{$active_theme->location}builder-assets/css/{$page_details->permalink}.css");
            $builder_css_path = asset("themes/{$active_theme->location}builder-assets/css/{$page_details->permalink}.css");
        @endphp

        @if (isActivePlugin('pagebuilder') && $page_details->page_type == 'builder' && file_exists($builder_css_file))
